...
public void _mm_write_SBYTES (byte buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){  _mm_write_SBYTE(buffer[pos++],fp); number--; } }
public void _mm_write_UBYTES2 (short buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){ _mm_write_UBYTE(buffer[pos++],fp); number--; } }

public void _mm_write_M_SWORDS (short buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){ _mm_write_M_SWORD(buffer[pos++],fp); number--; } }
public void _mm_write_M_UWORDS2 (int buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){ _mm_write_M_UWORD(buffer[pos++],fp); number--; } }
public void _mm_write_I_SWORDS (short buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){ _mm_write_I_SWORD(buffer[pos++],fp); number--; } }
public void _mm_write_I_UWORDS2 (int buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){ _mm_write_I_UWORD(buffer[pos++],fp); number--; } }

public void _mm_write_M_SLONGS (int buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){ _mm_write_M_SLONG(buffer[pos++],fp); number--; } }
public void _mm_write_M_ULONGS (int buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){ _mm_write_M_ULONG(buffer[pos++],fp); number--; } }
public void _mm_write_I_SLONGS (int buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){ _mm_write_I_SLONG(buffer[pos++],fp); number--; } }
...
public boolean _mm_read_M_SWORDS (short buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){ buffer[pos++]=_mm_read_M_SWORD(fp); number--; } return !isEOF(fp); }
public boolean _mm_read_M_UWORDS2 (int buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){ buffer[pos++]=_mm_read_M_UWORD(fp); number--; } return !isEOF(fp); }
public boolean _mm_read_I_SWORDS (short buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){ buffer[pos++]=_mm_read_I_SWORD(fp); number--; } return !isEOF(fp); }
public boolean _mm_read_I_UWORDS2 (int buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){ buffer[pos++]=_mm_read_I_UWORD(fp); number--; } return !isEOF(fp); }

public boolean _mm_read_M_SLONGS (int buffer[], int number, RandomAccessFile fp)  { int pos=0; while(number>0){ buffer[pos++]=_mm_read_M_SLONG(fp); number--; } return !isEOF(fp); }
