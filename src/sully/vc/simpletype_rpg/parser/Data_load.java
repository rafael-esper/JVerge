...
public class Data_load {
	...
	*/
	...
	static int LoadStructCast()
	{
		...
		try {
			File lsca_file = new File(load(dat_directory+"Cast.dat").toURI());
			BufferedReader fin = new BufferedReader(new FileReader(lsca_file)); // Attempts to open file in read mode
			...
			while (!strcmp("END_OF_FILE", lsca_word)) { // While the EOF token isn't found
				if (!strcmp("//", left(lsca_word, 2)) && len(lsca_string) >= MIN_LINE_LENGTH) // If the line is an entry not junk
				{
					...
					lsca_fgrowth = new File(load(cast_dat_directory+lsca_word).toURI());
					BufferedReader fing = new BufferedReader(new FileReader(lsca_fgrowth)); // Attempts to open the defined growth file
	...
	static int LoadStructClass()
	{
		...
		try {
			File lscl_file = new File(load(dat_directory+"Class.dat").toURI());
			BufferedReader fin = new BufferedReader(new FileReader(lscl_file)); // Attempts to open file in read mode
	...
	static int LoadStructEquip()
	{
		...
		try {
			File lseq_file = new File(load(dat_directory+"Equip.dat").toURI());
			BufferedReader fin = new BufferedReader(new FileReader(lseq_file)); // Attempts to open file in read mode
	...
	static int LoadStructSkillTypes()
	{
		...
		try {
			File lsst_file = new File(load(dat_directory+"Skilltypes.dat").toURI());
			BufferedReader fin = new BufferedReader(new FileReader(lsst_file)); // Attempts to open file in read mode
